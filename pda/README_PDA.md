...
<pre>
    pda/		: projet de développement PDA
...
Ensuite, il faut se placer dans le répertoire `pda/ww`


### En cours de développement

L'environnement est contrôlé par un script ANT ; il faut TOUJOURS se placer dans le répertoire `pda/ww`.
...
1. Lancer le serveur de messagerie en local : ALLER DANS LE REPERTOIRE pda/server et lancer le serveur `$ java -jar pdaServer.jar`
...
3. Lancement du test du client de messagerie : envois de messages de user1 à user2 (2 comptes déjà créés)
   ...
   DANS LE REPERTOIRE pda/ww, exécuter `$ ant runTestClient`
...
Le nom de cette machine (communiqué ultérieurement) est à renseigner dans le champ `<remoteHost>...</remoteHost>` du fichier de configuration en `pda/data/xml/pdaServer/configClient.xml`.
